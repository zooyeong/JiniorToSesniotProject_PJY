...
<body>
		...
		<div id="mailContainer">
			...
			<c:forEach var="item" items="${mail}">
				<div class="middleContainer">
					...
					<img src="resources/image/downarrow.png" class="clickBtn"
						onclick="mailDivView(event)">
				</div>
			</c:forEach>
						...
						<div class="mailDiv">
							...
							<div id="submitBtnContainer">
								<button type="submit" class="submitBtn"
									onclick="javascript: form.action='/acceptMail';">수락</button>
								<button type="submit" class="submitBtn"
									onclick="javascript: form.action='/refuseMail';">거절</button>
							</div>
							<p class="sendTime">${item.sendTime}</p>
							<input type="hidden" name="no" value="${item.no}"> <input
								type="hidden" name="revId" value="${item.sendId}"> <input
								type="hidden" name="sendId" value="${item.revId}"> <input
								type="hidden" name="matchingNumber"
								value="${item.matchingNumber}">
						</div>
						<img src="resources/image/downarrow.png" class="clickBtn"
							onclick="mailDivView(event)">
					</div>
	...
	<c:if test="${empty mail && userCode eq 'PAR'}">
		<script>
			emptyMailP();
		</script>
	</c:if>
	<c:if test="${empty mail && userCode eq 'SNR'}">
		<script>
			emptyMailS();
		</script>
	</c:if>
